...
const Page = () => {
  ...
  useEffect(() => {
    ...
    axios.get('http://127.0.0.1:8000/propel/employees')
      ...
      .catch(err => {
        console.error("Employee API error:", err.response?.data || err.message || err);
      });
  }, []);
    ...
    axios.get('http://127.0.0.1:8000/propel/register_holidays/')
      .then(res => {
        ...
        if (!Array.isArray(records)) {
          console.error("Expected an array in res.data.data, but got:", records);
          return;
        }
      ...
      .catch(err => {
        if (err.response) {
          console.error("Holiday API error:", err.response.data);
        } else if (err.request) {
