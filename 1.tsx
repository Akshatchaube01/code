...
const ExpandableUtilizationTable: React.FC<TableProps> = ({ columns, data }) => {
  ...
  return React.createElement('div', { className: 'overflow-x-auto rounded-xl border-2 border-red-600 shadow-sm p-4' }, 
    // Filter UI
    React.createElement('div', { className: 'mb-4' },
      React.createElement('p', { className: 'font-semibold mb-2' }, 'Select Countries to Display:'),
      React.createElement('div', { className: 'flex flex-wrap gap-4' },
        ...allCountries.map(country =>
          React.createElement('label', { key: country, className: 'flex items-center space-x-2' },
      ...
      React.createElement('tfoot', { className: 'bg-red-100 font-semibold text-red-800' },
        React.createElement('tr', null,
          ...
          ...totalColumn3.map((val, i) =>
            React.createElement('td', { key: `total-col3-${i}`, className: 'px-4 py-2' }, val)
          ),
