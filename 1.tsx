      ...
      <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 w-[700px] justify-between items-center z-[100]">
        {menus.map((menu, index) => (
          ...
          >
            ...
            {openMenu === menu.title && (
              <div className="absolute top-full left-8 bg-gray-700 mt-2 rounded-md shadow-lg min-w-max z-[100]">
                {menu.items.map((item, idx) => (
                  <Link
                    key={idx}
                    href={item.href}
                    className="flex items-center gap-2 px-5 py-3 hover:bg-gray-600 transition whitespace-nowrap z-[100]"
                  >
                    <span>{item.icon}</span>
                    {item.label}
                  </Link>
